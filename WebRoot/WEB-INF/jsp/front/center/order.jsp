...
<div class="mui-content">
    <!--头部导航-->
    <div class="mui-segmented-control mui_control">
        <p class="mui-control-item mui-active">全部</p>
        <p class="mui-control-item">待付款</p>
        <p class="mui-control-item">待发货</p>
        <p class="mui-control-item">待收货</p>
        <p class="mui-control-item">已完成</p>
    </div>
    <div style="height: 40px;"></div>

    <!--订单列表-->
    <div class="mescroll" id="mescroll">
        <div class="mui-control-content mui-active" id="dataList">

            <%--<div class="order_list dfk">
                <div class="order_tit">
                    <div class="order_tit_1">
                        <p><span>订单编号</span>：1266866565655</p>
                    </div>
                    <div class="order_tit_2">待付款</div>
                </div>
                <a href="front/to_orderDetail">
                    <div class="order_txt">
                        <div class="order_txt_1">
                            <div class="order_txt_pic">
                                <img src="static/front/images/banner.png"/>
                                <div class="width_pic">
                                    <div class="txt-cut">
                                        的师傅师傅的师傅师傅的说法的师傅师傅的说法的说法
                                    </div>
                                    <p>发荻港的烦恼</p>
                                    <div class="price"><span style="color: #999;">价格：</span>￥500</div>
                                </div>
                                <p>x1</p>
                            </div>
                        </div>
                ...
                <div class="order_bot">
                    <div>合计:<span>￥189.00</span></div>
                    <div>
                        <button type="button">取消订单</button>
                        <button type="button" class="ljfk"><a href="front/to_mallOrderDetails">立即支付</a></button>
                    </div>
                </div>
            </div>--%>
...
<script type="text/javascript" charset="utf-8">
    ...
    $(function () {
        ...
        /*初始化菜单*/
        var pdType = '';
        $(".mui_control p").click(function () {
            var status = $(this).text();
            if (pdType != status) {
                //更改列表条件
                pdType = status;
                /*$(".mui-control-item .active").removeClass("mui-active");
        ...
        function setListData(curPageData) {
            ...
            for (var i = 0; i < curPageData.length; i++) {
                ...
                var str = '';
                str += "<div class=\"order_list dfk\">\n" +
                    "        <div class=\"order_tit\">\n" +
                    "            <div class=\"order_tit_1\">\n" +
                    "                <p><span>订单编号</span>：" + pd.ORDER_NUMBER + "</p>\n" +
                    "            </div>\n" +
                    "            <div class=\"order_tit_2\">" + pd.STATUS + "</div>\n" +
                    "        </div>\n" +
                    "        <a href=\"front/to_orderDetail/" + pd.ORDER_ID + "\">\n" +
                    "            <div class=\"order_txt\">\n" +
                    "                <div class=\"order_txt_1\">" +
                    "                   <div class=\"order_txt_pic\">\n";

                for (let i = 0; i < pd.orderItem.length; i++) {
                    var orderItem = pd.orderItem[i];
                    // 多个商品只显示图片
                    if (pd.orderItem.length > 1) {
                        str += "      <img src=\"" + getLoading() + "\" imgurl=" + orderItem.PIC + ">\n" /*+
                            "                        <div class=\"width_pic\">\n" +
                            "                            <div class=\"txt-cut\">\n" +
                            // "                                " + pd.PROD_NAME + "\n" +
                            "                            </div>" +
                            // "                            <p>此处预留SKU名称</p>\n" +
                            "                            <div class=\"price\"><span style=\"color: #999;\"></div>\n" +
                            "                        </div>"*/;
                    } else {
                        str += "   <img src=\"" + getLoading() + "\" imgurl=" + orderItem.PIC + ">\n" +
                            "        <div class=\"width_pic\">\n" +
                            ...
                            "            </div>" +
                            // "            <p>此处预留SKU名称</p>\n" +
                            "            <div class=\"price\"><span style=\"color: #999;\">价格：</span>￥" + orderItem.PRICE + "</div>\n" +
                            "        </div>";
                    }
                }

                str += "     <p>x" + pd.PRODUCT_SUM + "</p>" +
                    "                </div>" +
                    "               </div>\n" +
                    "            </div>\n" +
                    "        </a>" +
                    ...
                    "        <div class=\"order_bot\">\n" +
                    "            <div>合计:<span>￥" + pd.ACTUAL_TOTAL + "</span></di v>";

                // 渲染订单状态对应的按钮
                if (pd.STATUS == '待付款') {
                    str += "         <div>\n" +
                        "                <button type=\"button\">取消订单</button>\n" +
                        "                <button type=\"button\" class=\"ljfk\"><a href=\"front/to_mallOrderDetails/" + pd.ORDER_ITEM_ID + "\">立即支付</a></button>\n" +
                        "            </div>\n" +
                        "        </div>\n" +
                        "    </div>";
                }
                if (pd.STATUS == '待收货') {
                    str += "         <div>\n" +
                        "                <button type=\"button\" class=\"ljfk\"><a onclick='signFor(" + pd.ORDER_ID + ")'>确认收货</a></button>\n" +
                        "            </div>\n" +
                        "        </div>\n" +
                        "    </div>";
                }
        ...
        function getListDataFromNet(pdType, pageNum, pageSize, successCallback, errorCallback) {
            ...
            var url = 'front/getUserOrderList.do?STATUS=' + pdType + '&currentPage=' + pageNum + "&showCount=" + pageSize;
