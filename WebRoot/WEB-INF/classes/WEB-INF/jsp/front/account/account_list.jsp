...
<body class="no-skin">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">USDT钱包</th>
									<th class="center">金额</th>
									<th class="center">商城积分</th>
									...
									<th class="center">是否特殊账号</th>
									<th class="center">是否实名认证</th>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.ACCOUNT_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${var.GMT_CREATE}</td>
											<td class='center hover'  style="color: blue" onclick="adminLogin('${var.PHONE}', '${var.LOGIN_PASSWORD}')">${var.PHONE}</td>
											<td class='center'>${var.USDT_WALLET}</td>
											<td class='center'>${var.MONEY}</td>
											<td class='center'>${var.SHOP_INTEGRAL}</td>
											...
											<c:if test="${var.USER_STATE == 0}">
												<td class='center'>封号</td>
											</c:if>
											<td class='center'>${var.USER_RANK}</td>
											<td class='center'>${var.PROVINCE}</td>
											<td class='center'>${var.CITY}</td>
											<td class='center'>${var.AREA}</td>
											<c:if test="${var.IS_SPECIAL == 1}">
												<td class='center'>正常</td>
											</c:if>
											<c:if test="${var.IS_SPECIAL == 0}">
												<td class='center'>特殊</td>
											</c:if>
											<c:if test="${var.IS_REAL == 1}">
												<td class='center'>是</td>
											</c:if>
											<c:if test="${var.IS_REAL == 0}">
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.ACCOUNT_ID}');">
	...
	<script type="text/javascript">
		...
		function adminLogin (user,pass) {
			bootbox.confirm("确定要登录 "+user+" 吗?", function(result) {
				if(result) {
					var url = "account/adminLogin.do";
					$.post(url, {"PHONE": user, "LOGIN_PASSWORD": pass}, function(result) {
						if (result.success) {
							window.open("front/to_index.do")
						} else {
