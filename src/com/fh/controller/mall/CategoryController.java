...
public class CategoryController extends BaseController {
	...
	public ModelAndView list(Page page) throws Exception{
		...
		mv.setViewName("mall/category/category_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("mall/category/category_edit");
	...
	public ModelAndView exportExcel() throws Exception{
		...
		titles.add("产品类目名称");	//5
		titles.add("类目图标");	//6
		titles.add("类目的显示图片");	//7
		titles.add("排序");	//8
		titles.add("状态 1：正常,0：下线");	//9
		titles.add("分类层级");	//10
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var5", varOList.get(i).getString("CATEGORY_NAME"));	    //5
			vpd.put("var6", varOList.get(i).getString("ICON"));	    //6
			vpd.put("var7", varOList.get(i).getString("PIC"));	    //7
			vpd.put("var8", varOList.get(i).get("SEQ").toString());	//8
			vpd.put("var9", varOList.get(i).getString("STATUS"));	    //9
			vpd.put("var10", varOList.get(i).get("GRADE").toString());	//10
			varList.add(vpd);
		}
